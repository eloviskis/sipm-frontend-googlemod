...
const Motivos = () => {
  ...
  useEffect(() => {
    const fetchMotivos = async () => {
      try {
        ...
        const motivosData = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  const handleAddMotivo = async () => {
    try {
      ...
      if (user) {
        const docRef = await addDoc(collection(db, 'motivos'), { name: newMotivo });
  ...
  return (
    <Container maxWidth="sm" sx={{ mt: 4, mb: 4 }}>
      ...
      <List>
        {motivos.map((motivo) => (
          <ListItem key={motivo.id} sx={{ mb: 1, backgroundColor: '#f9f9f9', borderRadius: 1 }}>
