...
import { Container, Typography, TextField, Button, List, ListItem, ListItemText, Alert } from '@mui/material';
...
const PreConsultations = () => {
  ...
  useEffect(() => {
    const fetchPreConsultations = async () => {
      try {
        ...
        const preConsultationsData = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  const handleAddPreConsultation = async () => {
    try {
      const docRef = await addDoc(collection(db, 'preConsultations'), { name: newPreConsultation });
      setPreConsultations([...preConsultations, { id: docRef.id, name: newPreConsultation }]);
    ...
    <Container maxWidth="sm" sx={{ mt: 4, mb: 4 }}>
      ...
      <List sx={{ mt: 2 }}>
        {preConsultations.map((preConsultation) => (
          <ListItem key={preConsultation.id} sx={{ mb: 1, backgroundColor: '#f9f9f9', borderRadius: 1 }}>
